...
test('About uses the approved packing image before the story and links to the catalogue', () => {
  ...
  expect(fs.existsSync(path.join(__dirname, '../../public', imagePath))).toBe(
    true
  );
  expect(aboutComponent).toContain(imagePath);
  expect(aboutComponent.indexOf(imagePath)).toBeLessThan(
    aboutComponent.indexOf('className="about-story"')
  );
