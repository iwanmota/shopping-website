...
describe('checkoutCart', () => {
  test('prices items on the server and atomically decrements sale and regular inventory', async () => {
    ...
    expect(receipt.items).toEqual([
      expect.objectContaining({ productId: 1, unitPrice: 149.99, subtotal: 299.98 }),
      expect.objectContaining({ productId: 2, unitPrice: 299.99, subtotal: 299.99 })
    ]);
    expect(receipt.total).toBe(599.97);
    expect(await get(db, 'SELECT onSaleQuantity FROM products WHERE id = 1')).toEqual({ onSaleQuantity: 1 });
    expect(await get(db, 'SELECT regularInventory FROM products WHERE id = 2')).toEqual({ regularInventory: 1 });
  ...
  test('rolls back every inventory change when any cart line is unavailable', async () => {
    ...
    await expect(checkoutCart(db, 7, [
      { productId: 1, quantity: 2, pricingTier: 'sale' },
      { productId: 2, quantity: 3, pricingTier: 'regular' }
    ])).rejects.toMatchObject({ status: 409 });

    expect(await get(db, 'SELECT onSaleQuantity FROM products WHERE id = 1')).toEqual({ onSaleQuantity: 3 });
    expect(await get(db, 'SELECT regularInventory FROM products WHERE id = 2')).toEqual({ regularInventory: 2 });
