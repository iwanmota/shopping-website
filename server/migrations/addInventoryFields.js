...
async function runMigration() {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run(
        `UPDATE products SET regularInventory = 20 WHERE regularInventory IS NULL OR regularInventory = 0`,
        (err) => {
          if (err) {
            reject(err);
            return;
    ...
    await new Promise((resolve, reject) => {
      db.run(
        `UPDATE products SET createdAt = ?, updatedAt = ? WHERE createdAt IS NULL OR updatedAt IS NULL`,
