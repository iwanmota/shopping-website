...
const PRODUCT_IMAGES_BASE_DIR = path.join(
  __dirname,
  '../public/images/products'
);
const PRODUCT_IMAGES_UPLOADS_DIR = path.join(
  PRODUCT_IMAGES_BASE_DIR,
  'uploads'
);
const PRODUCT_IMAGES_THUMBNAILS_DIR = path.join(
  PRODUCT_IMAGES_BASE_DIR,
  'thumbnails'
);
    ...
    if (oldImagePath && oldImagePath !== newImagePath) {
      ...
      try {
        ...
        if (deletionResult.errors.length > 0) {
          console.warn(
            'Errors during old image cleanup:',
        ...
        if (deletionResult.deletedFiles.length > 0) {
          console.log(
            'Successfully cleaned up old image files:',
