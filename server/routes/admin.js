...
router.post('/products', (req, res) => {
  ...
  if (parsedRegularInventory < 0) {
    return res
      .status(400)
      .json({ error: 'Regular inventory cannot be negative' });
  }
  ...
  if (isOnSale && (!salePrice || isNaN(salePrice) || salePrice <= 0)) {
    return res
      .status(400)
      .json({ error: 'Valid sale price is required for products on sale' });
  }
...
router.put('/products/:id', (req, res) => {
  ...
  db.get('SELECT * FROM products WHERE id = ?', [productId], (err, product) => {
    ...
    // Validate inventory fields
    const parsedRegularInventory =
      regularInventory !== undefined
        ? parseInt(regularInventory)
        : product.regularInventory;
    const parsedOnSaleQuantity =
      onSaleQuantity !== undefined
        ? parseInt(onSaleQuantity)
        : product.onSaleQuantity;
    ...
    if (parsedOnSaleQuantity < 0) {
      return res
        .status(400)
        .json({ error: 'Sale inventory cannot be negative' });
    }

    // Determine if product is on sale
    const productIsOnSale =
      isOnSale !== undefined ? !!isOnSale : !!product.isOnSale;

    // Validate sale price if product is on sale
    const parsedSalePrice =
      salePrice !== undefined ? parseFloat(salePrice) : product.salePrice;
    ...
    if (
      salePrice !== undefined ||
      (productIsOnSale && product.isOnSale === 0)
    ) {
        ...
        async function (err) {
          ...
          if (product.image) {
            ...
            try {
              ...
              responseData.imageCleanup.deletedFiles =
              ...
              if (deletionResult.errors.length > 0) {
                console.error(
                  `Image deletion errors for product ${productId}:`,
              ...
              if (deletionResult.deletedFiles.length > 0) {
                console.log(
                  `Successfully deleted image files for product ${productId}:`,
                  deletionResult.deletedFiles
                );
              }
            } catch (error) {
  ...
  db.get('SELECT * FROM products WHERE id = ?', [productId], (err, product) => {
    ...
    uploadSingleImage(req, res, async (uploadErr) => {
      ...
      try {
        ...
        const updateSql =
          'UPDATE products SET image = ?, updatedAt = ? WHERE id = ?';
        const updateParams = [
        ...
        db.run(updateSql, updateParams, async function (updateErr) {
          ...
          if (oldImagePath) {
            try {
              replacementResult = await replaceProductImage(
                oldImagePath,
                imageInfo.relativePath
              );
              ...
              if (replacementResult.errors.length > 0) {
                console.error(
                  `Image replacement errors for product ${productId}:`,
              ...
              if (replacementResult.oldImageCleanup.attempted) {
                if (
                  replacementResult.oldImageCleanup.success &&
                  replacementResult.oldImageCleanup.deletedFiles.length > 0
                ) {
                  console.log(
                    `Successfully replaced image for product ${productId}. Cleaned up files:`,
                    replacementResult.oldImageCleanup.deletedFiles
                  );
                } else if (
                  replacementResult.oldImageCleanup.errors.length > 0
                ) {
                  console.warn(
                    `Image replacement completed for product ${productId}, but cleanup had issues:`,
                    replacementResult.oldImageCleanup.errors
                  );
                }
              }
            } catch (error) {
              console.error(
                `Error during image replacement for product ${productId}:`,
                error
              );
